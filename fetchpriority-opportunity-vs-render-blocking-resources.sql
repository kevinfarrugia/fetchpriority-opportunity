#standardSQL
# Correlation between render-blocking resources and opportunity_ms
#
# Notes:
# - LCP is an image
...
SELECT
  device,
  number_of_render_blocking_assets,
...
FROM (
  SELECT
    device,
    IF(
      number_of_render_blocking_scripts + number_of_render_blocking_stylesheets >= 20,
      20,
      number_of_render_blocking_scripts + number_of_render_blocking_stylesheets
    ) AS number_of_render_blocking_assets,
    opportunity_ms
  FROM (
    SELECT
      device,
